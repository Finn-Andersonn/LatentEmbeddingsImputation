...
# However, we can consider adding features 
# that provide more context about the missing data patterns and enhance imputation, like an embeddings’ ability to capture similarity
...
#      -> Population Density
#      -> should help the graph construction (e.g., weighting edges by port distance) and embeddings (e.g., tanks near ports might have similar operational patterns).
# NOTE: Adding too many features runs risk of curse of dimensionality (mentioned and monitored in corr_utilities.py)
# -------------------------------------------------------
import pandas as pd
...
class WeatherData:
    '''
    Description:
        parse loc and lat, get tank locations, see what weather stations are nearby, have a fallback
        as some stations dont record everything (this is within a reasonable distance)
    '''
...
class EnhancedAdditionalFeatures:
    ...
    Description:
        ...
        "Enhanced" as this was difficult.
    
    Args:
        LOUISIANA_PORTS: manually took the louisiana port locations -> inspired by map.py
    ...
    def _add_coordinates(self):
        '''
        Parse coordinates from Location column
        '''
    ...
    def _fetch_ports_from_osm(self, bbox):
        '''
        Description:
    ...
    def add_eia_report_features(self):
        '''
        Description:
            Add features related to EIA reporting cycles -> may add more insight to relation with other tanks:
                -> days_since_last_eia_report (weekly/monthly)
                -> days_until_next_eia_report
                -> is_eia_report_week (bool)

            [DEPRECATED] -> I used monthly so days since EIA report doesnt work as our dates are always the same each month.
        '''
        print("Adding EIA report features...")
        
        # all EIA report dates in the dataset's time range
        ...
        # each row, find nearest report date
    ...
    def add_spatial_features(self, ports = None):
        """
        Description:
    ...
    def add_crude_oil_prices(self, excel_path: str, sheet_name: str = "Data 1"):
        ...
        Args:
            excel_path: Path to the local XLS(X) file with daily WTI/Brent data.
