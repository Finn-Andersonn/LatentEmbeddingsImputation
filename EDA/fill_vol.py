...
class fill_vol():
    ...
    def ranked_tank_farm_map(self, save_path='ranked_tank_farms_map.png', with_basemap=True):
        plt.style.use('default')
        matplotlib.rcParams.update(matplotlib.rcParamsDefault)
        df = self.df.copy()
        coords = df['Location'].str.extract(r'POINT\(\s*(?P<lon>[-+]?\d*\.\d+)\s+(?P<lat>[-+]?\d*\.\d+)\s*\)').astype(float)
        df['lon'] = coords['lon']
        df['lat'] = coords['lat']
        ...
        max_capacity_per_farm = max_per_tank.groupby('tank farm').agg({'tank volume': 'max','lat': 'mean','lon': 'mean'}).sort_values(by='tank volume', ascending=False).reset_index()
        ...
        max_vol = max_capacity_per_farm['tank volume'].max()
        min_vol = max_capacity_per_farm['tank volume'].min()        
        sizes = 400 + 700 * (max_capacity_per_farm['tank volume'] - min_vol) / (max_vol - min_vol)
        
        ranks = max_capacity_per_farm['Rank']
        norm = plt.Normalize(vmin=1, vmax=len(ranks))
        cmap = plt.cm.Reds_r  # Note the _r for reversed
        ...
        sc = ax.scatter(
            ...
            s=sizes,
            c=colors,
            edgecolors='darkred',  # Darker border for contrast
            linewidths=1.5,
            alpha=0.85,  # Slightly transparent for overlap visibility
            zorder=3
        )
        ...
        for idx, row in gdf.iterrows():
            t = ax.text(
                ...
                row.geometry.y,
                f"#{row['Rank']}",
                fontsize=max(10, min(16, sizes[idx]/60)),
                ha='center',
                va='center',
                color='white',
                fontweight='black',
        ...
        try:
            ...
            adjust_text(text_objects, 
                    arrowprops=dict(arrowstyle='->', color='black', lw=1, shrinkA=5),
                    force_text=0.5, force_objects=0.5,
                    expand_text=(1.2, 1.2),
                    #expand_points=(1.2, 1.5), 
                    lim=0.05,
                    max_iterations=50)  # Expand around points
        except ImportError:
        ...
        cbar.set_label('Rank (1 = Largest Capacity)', fontsize=12)
        cbar.ax.invert_yaxis()  # To match reversed colormap

        if with_basemap:
            cx.add_basemap(ax, source=cx.providers.OpenStreetMap.Mapnik, zorder=0)

        plt.savefig(save_path, dpi=350, bbox_inches='tight')  # Higher DPI
    ...
    def freq_tank_farm_map(self, save_path='freq_tank_farms_map.png', with_basemap=True):
        ...
        # Use a constant circle radius for all farms
        constant_radius = 8000  # adjust as needed (in EPSG:3857 meters)
        max_capacity_per_farm['radius'] = constant_radius
        
        # Compute wedge fill fraction (recording_count relative to max)
        ...
        # Draw circles and wedges for each tank farm
        for idx, row in gdf.iterrows():
        ...
        for idx, row in gdf_sorted.iterrows():
            ...
            for angle in angles:
                ...
                for distance_factor in [1.0, 1.3, 1.6]:  # Try different distances
                    ...
                    # 1. Distance to other labels (higher is better)
                    # 2. Distance to circle centers (higher is better)
                    # 3. Whether the label overlaps any circle (avoid)
                    
                    # Start with a base score
                    score = 0
                    
                    # 1. Check distance to other labels
                    ...
                    if label_positions:
                        ...
                        score += min_dist_to_label / 10000  # Scale factor
                    else:
                        # If no labels yet, give a good score
                        score += 10
                    
                    # 2. Check distance to circle centers (other than this circle)
                    ...
                    for circ_center, circ_radius in circle_positions:
                        ...
                        # Check if label overlaps this circle
